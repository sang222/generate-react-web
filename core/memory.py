...
def find_runs_by_task(task: str, limit: int = 10) -> List[Dict[str, Any]]:
    ...
    cursor = (
        collection.find(
            {"task": {"$regex": re.escape(task), "$options": "i"}},
            {"_id": 0},
        )
        .sort("created_at", -1)
        .limit(limit)
...
def find_failed_runs(limit: int = 20) -> List[Dict[str, Any]]:
    collection = get_runs_collection()
    cursor = (
        collection.find(
            {
                "$or": [
                    {"final_decision": {"$ne": "DONE"}},
                    {"release_status": {"$ne": "DONE"}},
                    {"severity": "BLOCKER"},
                ]
            },
            {"_id": 0},
...
def summarize_similar_runs(task: str, limit: int = 5) -> str:
