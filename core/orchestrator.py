...
OUTPUT_PROJECT_DIR = MODULE_CONFIG.get("devteam", {}).get(
    "output_project_dir",
    "output_project",
)
...
def _initialize_story_context(
    ...
    resolved_story_id = (story_id or "story_1").strip() or "story_1"
    resolved_story_name = (story_name or resolved_story_id).strip() or resolved_story_id
    resolved_project_id = (project_id or f"project_{uuid.uuid4().hex[:8]}").strip()
    resolved_epic_id = (epic_id or f"{resolved_project_id}-epic").strip()
    resolved_depends_on = depends_on or []

    epic_context = ensure_epic_context(
        resolved_project_id,
        resolved_epic_id,
        resolved_story_id,
        resolved_story_name,
    ...
    story_def = get_story_definition(resolved_project_id, resolved_story_id)
    delivery_index = ensure_delivery_index(
        resolved_project_id,
        resolved_epic_id,
        epic_context,
    )
    ensure_artifact_locks(resolved_project_id)
    ownership_map = ensure_ownership_map(resolved_project_id)
    ownership_path = str(ownership_map_path(resolved_project_id))

    inferred_baseline = resume_from or find_baseline_from_dependencies(
        resolved_project_id,
        resolved_story_id,
        DELIVERIES_DIR,
    )
    ...
    story_packet["depends_on"] = resolved_depends_on or story_packet.get("depends_on", [])
    story_packet["acceptance_criteria"] = story_def.get(
        "acceptance_criteria",
        story_packet.get("acceptance_criteria", []),
    )
    ...
    context = _base_context(
        ...
        story_name=resolved_story_name,
...
def _run_pm_phase(context: Dict[str, Any], memory_manager: AgentMemoryManager) -> None:
    pm_ctx = memory_manager.load_context(
        "pm",
        context["task"],
        build_workflow_context(context, SYSTEM_TARGET),
    )
    context["prd"] = run_pm(
        task=(
            f"{context['task']}\n\nStory packet:\n"
            f"{json.dumps(context['story_packet'], ensure_ascii=False, indent=2)}"
        ),
        agent_context=pm_ctx,
    )

    context["gate_state"] = pass_gate(
        context["gate_state"],
        "GATE_1_RESEARCH",
        "Research/brief context captured.",
        ["docs/brief.md"],
    )
    context["gate_state"] = pass_gate(
        context["gate_state"],
        ...
        [
            "docs/prd.md",
            f"project_state/{context['project_id']}/epic_context.json",
        ],
    )
...
def _run_architecture_phase(
    context: Dict[str, Any],
    memory_manager: AgentMemoryManager,
    ownership_map: Dict[str, Any],
) -> None:
    architect_ctx = memory_manager.load_context(
        "architect",
        context["task"],
        build_workflow_context(context, SYSTEM_TARGET),
    )
...
def _handle_brownfield_readiness(
    ...
    impact_report = brownfield.get("impact_report", {})
    readiness_report = brownfield.get("readiness_report", {})
    ...
    context["story_packet"].update(
        {
            ...
            "allowed_change_scope": impact_report.get("affected_modules", []),
            "forbidden_change_scope": impact_report.get("protected_modules", []),
    ...
    context["execution_error"] = (
        "[brownfield readiness failed]\nMissing information: "
        + ", ".join(readiness_report.get("missing_information", []))
    )
    ...
    log_step("readiness", f"Finished with final decision: {context['final_decision']}")
    return context
...
def _merge_and_check_outputs(
    ...
    context["fix_suggestion"] = (
        "Generate or preserve the minimum required file set before adding extra features."
    )
    context["bugs"] = context["qa_detail"]["structural_bugs"]
    context["release_status"], context["severity"], reason = classify_release_status(context)
    run_lead_and_store_summary(context, memory_manager, reason, MAX_LOOP, SYSTEM_TARGET)
...
def _run_reviews(
    ...
    integration_review: ReviewResult = (
        _as_review_result(
            run_lane_review(
                context,
                memory_manager,
    ...
    context["fe_review"] = dict(fe_review)
    context["be_review"] = dict(be_review)
    context["integration_review"] = dict(integration_review)

    context["qa_detail"] = merge_qa_details(
        fe_review,
        be_review,
        integration_review,
    )
    ...
    context["fix_suggestion"] = _join_fix_suggestions(
        fe_review,
        be_review,
        integration_review,
    )
...
def _finalize_loop(
    ...
    if context["release_status"] == "DONE":
        context["gate_state"] = pass_gate(
            ...
            "Implementation, reviews, and integration checks passed.",
            ["output_project"],
        )
        context["gate_state"] = pass_gate(
            context["gate_state"],
            "RELEASE_GATE",
            "Release checklist approved for current story.",
            ["output_project"],
        )
        save_gate_state(context["gate_state"])
        return True
    ...
    context["gate_state"] = fail_gate(
        context["gate_state"],
        "GATE_4_IMPLEMENTATION",
        context.get("execution_error", "") or "Implementation gate failed.",
    )
    save_gate_state(context["gate_state"])
    write_workflow_status(context, "Retry required for the current story.")
    return False
...
def run_orchestrator(
    ...
    context["first_breath"] = memory_manager.ensure_first_breath(
        task,
        build_workflow_context(context, SYSTEM_TARGET),
    )
    context["gate_state"] = load_gate_state(
        context["project_id"],
        context["epic_id"],
        context["story_id"],
    )
    ...
    if context["release_status"] == "DONE":
        ...
        write_workflow_status(
            context,
            "Current story delivered. Move to the next ready story if any.",
        )
    ...
    save_run(context)
    memory_manager.remember_run(context)
    log_step("run_orchestrator", f"Finished with final decision: {context['final_decision']}")
    return context